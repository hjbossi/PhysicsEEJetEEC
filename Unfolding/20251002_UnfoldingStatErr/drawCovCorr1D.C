...
void drawCovCorr1D(){
   ...
   for(int i = 0; i <= BinCount; i++){
    ...
    TFile* inFile = TFile::Open("cov_matrix_09122026.root");
    
    TH2D* cov_Z = (TH2D*)inFile->Get("cov_Z"); 
    ...
    c1->SaveAs("cov_Z_09122026.pdf"); 
    
    
    TH2D* corr_Z = (TH2D*)inFile->Get("corr_Z");
    ...
    corr_Z->Draw("colz0"); 
    c2->SaveAs("corr_Z_09122026.pdf"); 
    
    // now make a plot to compare the statistical uncertainties 
    TH1D* statErr = (TH1D*)inFile->Get("stat_error_Z");
    int nEvents = 1333529; 

    
    // now for the purposes of comparison, we want the statistical error on the raw data
    TFile* unfoldingFile = TFile::Open("unfoldingE2C_DataUnfolding_StatErrCheck_WithCovariance01292026.root"); 
    TH2D* raw = (TH2D*)unfoldingFile->Get("r_Z"); 
    ...
    leg->Draw();
    c3->SaveAs("EEC_statErr_PreUnfolding_09122026.pdf");
