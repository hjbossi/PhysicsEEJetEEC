...
int main(int argc, char *argv[]){
   ...
   for(int i = 0; i <= BinCount; i++)
   {
    ...
    TH2D *hUnfoldedNom = (TH2D *)Files[0]->Get(Form("Bayesian_Unfoldediter%d_Theta", iterNom));
    hUnfoldedNom->SetName("hUnfoldedNom");

    TH1D *Hnom = (TH1D*)hUnfoldedNom->ProjectionX();
    Hnom->Reset();
    for (int i = 1; i <= hUnfoldedNom->GetNbinsX(); ++i) {
        double weight = 0;
        double error = 0;
        for (int j = 1; j <= hUnfoldedNom->GetNbinsY(); ++j) {
           double binContent = hUnfoldedNom->GetBinContent(i, j);
           double binError= hUnfoldedNom->GetBinError(i,j); // manully enforce poisson errors //hUnfolded->GetBinError(i,j);
           double binCenter = hUnfoldedNom->GetYaxis()->GetBinCenter(j);
           weight += binContent*((binCenter));
            error += pow(binError*binCenter, 2);

        }
        Hnom->SetBinContent(i, weight);
        Hnom->SetBinError(i, sqrt(error));
    }
    DivideByBin(*Hnom, Bins);
    Hnom->Scale(1.0 / Hnom->Integral());
    Hnom->SetDirectory(0);
    Hnom->SetStats(0);
    Hnom->SetTitle("");
    Hnom->GetXaxis()->SetTitle("#theta_{L}");
    Hnom->GetYaxis()->SetTitle("#frac{1}{N_{event}} #frac{d(Sum E_{i}E_{j}/E^{2})}{d #theta_{L}}");
    Hnom->SetMarkerColor(Colors[5]);
    Hnom->SetMarkerStyle(20);
    Hnom->SetLineColor(Colors[5]);
    Hnom->SetLineWidth(2);

    Histograms.push_back(Hnom);


    for(int i = 1; i < 4; i++){
        std::cout << "On file " << i << std::endl;
        TH2D *hUnfolded = (TH2D *)Files[0]->Get(Form("Bayesian_Unfoldediter%d_Theta", i));
        if(hUnfolded == NULL){
          cerr << "Histogram not found" << endl;
          return 0;
        }
        hUnfolded->SetName(Form("UnfoldedIter_%d", i));
        ...
        for (int i = 1; i <= hUnfolded->GetNbinsX(); ++i) {
            ...
            for (int j = 1; j <= hUnfolded->GetNbinsY(); ++j) {
               ...
               double binError= hUnfolded->GetBinError(i,j); // manully enforce poisson errors //hUnfolded->GetBinError(i,j);
   ...
   std::cout << "Plotting the distribution" << std::endl;
    MakeCanvas(Histograms, Labels, output + Prefix + "EEC2","#theta_{L}", "#frac{1}{N_{event}} #frac{d(Sum E_{i}E_{j}/E^{2})}{d #theta_{L}}", 2e-4, 0.08,
         DoRatio, true);
   ...
   string X, string Y, double WorldMin, double WorldMax, bool DoRatio, bool LogX)
{
   ...
   Latex.SetTextColor(kBlack);
   if(DoRatio)
      Latex.DrawLatex(MarginL * 0.3, MarginB + PadRHeight * 0.5, "Ratio to iter 4");
   ...
   Latex.DrawLatex(0.01, 0.01, "Finalization of Results - April 24th (HB)");

   TLegend Legend(0.15, 0.88, 0.35, 0.88 - 0.035 * min(NLine, 4));
   ...
   TLegend Legend2(0.7, 0.88, 0.9, 0.88 - 0.035 * (NLine - 4));
